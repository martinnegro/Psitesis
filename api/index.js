...
conn.sync({ force: true }).then(() => {
	server.listen(process.env.PORT || 3001, async () => {
		...
		//provando notificaciones
		const userT = await User.findOne({
			where: { user_id_A0: 'google-oauth2|110400245480152722998' },
		});
		...
		const notifications = [{
			...
			receiverId: userT.user_id,
			senderId: userT.user_id,
		},{
			id: uuidv4(),
			link: '/user/google-oauth2|110400245480152722998',
			description: 'descripcion',
